...
const Navbar = () => {
    ...
    return (
        <div className='max-w-screen-xl mx-auto h-20 py-6 bg-white-800 flex items-center justify-between border-b-[1px] border-zinc-700'>
            <div className='n-left flex items-center h-20'>
                <svg width="20%" height="35%" viewBox="0 0 15 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className='links flex items-center ml-10 gap-16'>
                    {["Home", "Work", "Culture", "", "News"].map((ele, index) => {
                        return (
                            ele.length === 0 ? <span key={index} className='w-[1px] h-7 bg-zinc-700'></span> :
                                (<a href="#" onMouseEnter={(e) =>{setisHoverIndex(e._targetInst.key)}} key={index}  className='text-sm flex items-center gap-2'>
                                    {isHoverIndex == index && <span style={{ boxShadow: "0 0 0.25rem #00FF19" }} className='w-1.5 h-1.5 bg-green-500 rounded-full'></span>}
                                    {ele}
                                </a>)
                        )
                    })}
